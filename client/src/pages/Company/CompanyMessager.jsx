...
function CompanyMessager() {
    ...
    // Sync conversations from store to local state
    useEffect(() => {
    ...
    // Helper function để lấy thông tin member khác trong conversation (user)
    const getOtherMember = (conversation) => {
        if (!conversation || !conversation.members) return null;

        // Tìm member là user (không phải company)
    ...
    // Lấy company member để kiểm tra tin nhắn của mình
    const getMyMember = (conversation) => {
        if (!conversation || !conversation.members) return null;
        return conversation.members.find((member) => member.memberType === 'company');
    };
    ...
    const handleConversationClick = async (conversation) => {
        ...
        navigate(`/company/messager/${conversation._id}`);
    };
    ...
    // Kiểm tra tin nhắn có phải của company (mình) không
    const isMyMessage = (msg) => {
        // Nếu senderType là company thì là tin nhắn của mình
        return msg.senderType === 'company';
    };
    ...
    const handleSendMessage = async () => {
        if (message.trim() && selectedConversation) {
            const data = {
                ...
                senderType: 'company', // Company gửi tin nhắn
            };
    ...
    return (
        <div className="h-[80vh] flex bg-white rounded-lg shadow-sm overflow-hidden">
            {/* Sidebar - Danh sách ứng viên */}
            <div className="w-80 border-r border-gray-200 flex flex-col">
                {/* Header Sidebar */}
                <div className="p-4 border-b border-gray-200">
                    <h2 className="text-xl font-bold text-gray-800 mb-3">Tin nhắn ứng viên</h2>
                    <Input
            ...
            <div className="flex-1 flex flex-col">
                {selectedConversation && otherMemberInSelected ? (
                            ...
                            <div className="space-y-4">
                                {listMessager && listMessager.length > 0 ? (
                                    ...
                                    <div className="flex items-center justify-center h-full text-gray-400">
                                        Chưa có tin nhắn nào. Hãy gửi lời chào tới ứng viên!
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex items-center justify-center bg-gray-50">
                        <Empty
                            description={<span className="text-gray-500">Chọn một ứng viên để bắt đầu trò chuyện</span>}
                        />
                    </div>
                )}
            </div>
...
export default CompanyMessager;
